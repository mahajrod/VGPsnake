rule gfa2fasta:
    input:
        gfa=output_dict["contig"] / ("{assembler}/%s.contig.{assembler}.{haplotype}_ctg.gfa" % config["genome_name"])
    output:
        fasta=output_dict["contig"] / ("{assembler}/%s.contig.{assembler}.{haplotype}.fasta" % config["genome_name"])
    log:
