...
rule cutadapt_illumina:
    input:
        forward_fastq=output_dict["data"] / ("fastq/{datatype}/raw/{pairprefix}%s%s" % (input_forward_suffix_dict["{datatype}"],
                                                                                     config["fastq_extension"])),
        reverse_fastq=output_dict["data"] / ("fastq/{datatype}/raw/{pairprefix}%s%s" % (input_reverse_suffix_dict["{datatype}"],
                                                                                      config["fastq_extension"])),
    output:
    ...
    params:
        error_rate=lambda wildcards: "-e {0} ".format(parameters["tool_options"]["cutadapt"]["{datatype}"]["error_rate"]) if "error_rate" in parameters["tool_options"]["cutadapt"]["{datatype}"] else "",
        min_read_length=lambda wildcards: " -m {0} ".format(parameters["tool_options"]["cutadapt"]["{datatype}"]["min_read_length"]) if "min_read_length" in parameters["tool_options"]["cutadapt"]["{datatype}"] else "",
        min_adapter_length=lambda wildcards: " --overlap {0} ".format(parameters["tool_options"]["cutadapt"]["{datatype}"]["min_adapter_length"]) if "min_adapter_length" in parameters["tool_options"]["cutadapt"]["{datatype}"] else "",
        adapter_match_times=lambda wildcards: " --times {0}".format(parameters["tool_options"]["cutadapt"]["{datatype}"]["adapter_match_times"]) if "adapter_match_times" in parameters["tool_options"]["cutadapt"]["{datatype}"] else "",
        check_read_rc= lambda wildcards: " --rc " if ( ("check_read_rc" in parameters["tool_options"]["cutadapt"]["{datatype}"]) and  parameters["tool_options"]["cutadapt"]["{datatype}"]["check_read_rc"]) else "",
        discard_trimmed= lambda wildcards: " --discard-trimmed " if ( ("discard_trimmed" in parameters["tool_options"]["cutadapt"]["{datatype}"]) and parameters["tool_options"]["cutadapt"]["{datatype}"]["discard_trimmed"]) else "",
        forward_anywhere_adapters= lambda wildcards: (" -b " + " -b ".join(parameters["tool_options"]["cutadapt"]["{datatype}"]["forward_anywhere_adapter_list"])) if "forward_anywhere_adapter_list" in parameters["tool_options"]["cutadapt"]["{datatype}"] else "",
        reverse_anywhere_adapters= lambda wildcards: (" -B " + " -B ".join(parameters["tool_options"]["cutadapt"]["{datatype}"]["reverse_anywhere_adapter_list"])) if "reverse_anywhere_adapter_list" in parameters["tool_options"]["cutadapt"]["{datatype}"] else "",
        forward_three_prime_adapters= lambda wildcards: (" -a " + " -a ".join(parameters["tool_options"]["cutadapt"]["{datatype}"]["forward_three_prime_adapter_list"])) if "forward_three_prime_adapter_list" in parameters["tool_options"]["cutadapt"]["{datatype}"] else "",
        reverse_three_prime_adapters= lambda wildcards: (" -A " + " -A ".join(parameters["tool_options"]["cutadapt"]["{datatype}"]["reverse_three_prime_adapter_list"])) if "reverse_three_prime_adapter_list" in parameters["tool_options"]["cutadapt"]["{datatype}"] else "",
        forward_five_prime_adapters= lambda wildcards: (" -g " + " -g ".join(parameters["tool_options"]["cutadapt"]["{datatype}"]["forward_five_prime_adapter_list"])) if "forward_five_prime_adapter_list" in parameters["tool_options"]["cutadapt"]["{datatype}"] else "",
        reverse_five_prime_adapters= lambda wildcards: (" -G " + " -G ".join(parameters["tool_options"]["cutadapt"]["{datatype}"]["reverse_five_prime_adapter_list"])) if "reverse_five_prime_adapter_list" in parameters["tool_options"]["cutadapt"]["{datatype}"] else "",
    log:
