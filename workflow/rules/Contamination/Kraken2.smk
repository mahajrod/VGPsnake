...
rule kraken2: #
    input:
        ...
        forward_fastq=lambda wildcards: expand(output_dict["data"] / ("fastq/%s/filtered/{pairprefix}%s" % (wildcards.datatype,
                                                                                                              config["fastq_extension"])),
        ...
        reverse_fastq=lambda wildcards: expand(output_dict["data"] / ("fastq/%s/filtered/{pairprefix}%s" % (wildcards.datatype,
                                                                                                              config["fastq_extension"])),
                                               pairprefix=input_pairprefix_dict[wildcards.datatype],
                                               allow_missing=True) if wildcards.datatype in config["paired_fastq_based_data"] else [],
    ...
    params:
        forward_fastq=lambda wildcards: " <(cat {0} )".format(" ".join(expand(output_dict["data"] / ("fastq/%s/filtered/{pairprefix}%s" % (wildcards.datatype,
                                                                                                              config["fastq_extension"])),
                                                                       pairprefix=input_pairprefix_dict[wildcards.datatype],
                                                                       allow_missing=True) )) if wildcards.datatype in config["paired_fastq_based_data"] else "",
        reverse_fastq=lambda wildcards: " <(cat {0} )".format(" ".join(expand(output_dict["data"] / ("fastq/%s/filtered/{pairprefix}%s" % (wildcards.datatype,
                                                                                                              config["fastq_extension"])),
                                                                       pairprefix=input_pairprefix_dict[wildcards.datatype],
                                                                       allow_missing=True) )) if wildcards.datatype in config["paired_fastq_based_data"] else "",
