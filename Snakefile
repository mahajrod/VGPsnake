...
results_dict["contig"] = [*results_dict["filtering"],
                          ...
                          expand(output_dict["contig"] / ("{assembler}/%s.contig.{assembler}.pacbio.hic.{haplotype}_ctg.fasta" % config["genome_name"]),
                                 ...
                                 assembler=["hifiasm",],),
                          expand(output_dict["assembly_qc"] /("{assembly_stage}/busco5/{assembler}/{haplotype}/%s.{assembly_stage}.{assembler}.pacbio.hic.{haplotype}/short_summary.specific.%s.%s.{assembly_stage}.{assembler}.pacbio.hic.{haplotype}.txt"
                                                   % (config["genome_name"],
